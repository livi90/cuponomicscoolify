-- Primero verificamos si la tabla ya existe y la eliminamos si es necesario
DROP TABLE IF EXISTS tracking_conversions CASCADE;
...
CREATE INDEX idx_tracking_conversions_owner_id ON tracking_conversions(owner_id);
CREATE INDEX idx_tracking_conversions_store_id ON tracking_conversions(store_id);
CREATE INDEX idx_tracking_conversions_click_id ON tracking_conversions(click_id);
...
CREATE INDEX idx_tracking_conversions_coupon_id ON tracking_conversions(coupon_id);
CREATE INDEX idx_tracking_conversions_order_id ON tracking_conversions(order_id);
CREATE INDEX idx_tracking_conversions_utm_source ON tracking_conversions(utm_source);
CREATE INDEX idx_tracking_conversions_utm_campaign ON tracking_conversions(utm_campaign);
...
COMMENT ON COLUMN tracking_conversions.click_id IS 'Referencia al click original (BIGINT para compatibilidad)';
